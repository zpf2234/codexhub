...
const artifactTypeLabels = { 'skill-metadata': 'Skill metadata', 'plugin-config-entry': 'Configured plugin', 'marketplace-plugin-reference': 'Marketplace plugin', 'plugin-skills-reference': 'Bundled skill path', 'plugin-mcp-reference': 'Bundled MCP server configuration', 'plugin-app-reference': 'Bundled MCP app mapping', 'plugin-hook-reference': 'Bundled hook configuration', 'mcp-app': 'MCP app mapping', 'mcp-server-manifest': 'MCP server manifest', mcp: 'MCP configuration', 'mcp-registry': 'MCP Registry', 'mcp-config-entry': 'Configured MCP server', 'hook-config-entry': 'Configured hook event', 'agent-role-entry': 'Configured agent role', 'skill-config-entry': 'Skill configuration', 'codex-config': 'Codex configuration', 'agent-config': 'Custom agent', rule: 'Execpolicy rule', 'action-prompt': 'Codex Action prompt', 'custom-prompt': 'Custom prompt' };
const statusLabels = { verified: 'Verified', discovered: 'Deferred', registry: 'Registry', unknown: 'Unknown' };
const verificationLabels = { configured: 'Configured', 'manifest-reference': 'Declared', 'code-search': 'Code match', deferred: 'Deferred', passed: 'Verified', failed: 'Invalid', unavailable: 'Unavailable', registry: 'Registry' };
...
function artifactCard(artifact) {
  ...
  header.append(el('span', `category-badge category-${artifact.category}`, artifact.categoryLabel || artifactTypeLabels[artifact.artifactType] || labels[artifact.category] || artifact.category), el('span', `verification verification-${artifact.status}`, verificationLabels[artifact.verification] || statusLabels[artifact.status] || artifact.verification || artifact.status));
