...
For operational recovery, `GITHUB_DISCOVERY_TIMEOUT_MS` and `GITHUB_DISCOVERY_RETRIES` bound individual GitHub requests. The scheduled job processes one GitHub source, verifies at most five repository trees, and reads three MCP Registry pages per day. `DISCOVERY_MAX_SOURCES_PER_RUN`, `DISCOVERY_MAX_REPOSITORIES`, and `MCP_REGISTRY_MAX_PAGES` tune those batches; the checkpoint stores source completion plus repository/cursor positions so later runs continue the same cycle. Repository trees use bounded concurrency configured by `DISCOVERY_SCAN_CONCURRENCY`. `DISCOVERY_SOURCES=github-topic-codex-skills` can isolate one source while diagnosing a failed scheduled run.
