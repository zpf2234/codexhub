...
The scheduled discovery job publishes `api/v1/discovery/discovery.json` and its split views. Discovery records are candidates, not reviewed catalog entries. Each artifact has a stable source-scoped ID and records its repository, path, type, verification result, and provenance. `coverage.json` reports every declared source query, page/partition count, truncation, rate-limit or transport error, and whether the crawl is complete for that time and scope.
